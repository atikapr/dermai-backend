...
# 1. MODEL TFLITE
MODEL_PATH = "skin_cancer_image_model.tflite"
...
def predict():
    try:
        ...
        age_norm = (age - 5.0) / (85.0 - 5.0)
        age_norm = max(0.0, min(1.0, age_norm))  # Pastikan selalu di rentang 0-1
        ...
        meta_array = np.array(
            [[age_norm, sex, loc_head, loc_lower, loc_oral, loc_upper]],
            dtype=np.float32,
        )
